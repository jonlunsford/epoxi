defmodule Epoxi.PipelineMonitor do
  ...
  @spec get_cluster_stats() :: %{
    ...
    health_summary: %{healthy: non_neg_integer(), unhealthy: non_neg_integer(), unknown: non_neg_integer()},
  ...
  """
  @spec start_pipeline_optimal(PipelinePolicy.t(), atom()) :: {:ok, {atom(), pid()}} | {:error, String.t()}
  ...
  """
  @spec stop_pipeline_by_routing_key(String.t()) :: {:ok, non_neg_integer()} | {:error, String.t()}
  ...
  """
  @spec rebalance_cluster(atom()) :: {:ok, %{moved: non_neg_integer(), started: non_neg_integer(), stopped: non_neg_integer()}} | {:error, String.t()}
  ...
  defp identify_load_imbalance(health_results, cluster, ip_pool) do
    load_distribution = analyze_load_distribution(health_results)
    pool_nodes = Cluster.find_nodes_in_pool(cluster, ip_pool) |> Enum.map(& &1.name) |> MapSet.new()
    ...
    if length(pool_loads) < 2 do
      ...
      avg_load = pool_loads |> Enum.map(fn {_node, load} -> load.avg_load end) |> Enum.sum() / length(pool_loads)
      
      overloaded = Enum.filter(pool_loads, fn {_node, load} -> load.avg_load > avg_load * 1.5 end)
      underloaded = Enum.filter(pool_loads, fn {_node, load} -> load.avg_load < avg_load * 0.5 end)
